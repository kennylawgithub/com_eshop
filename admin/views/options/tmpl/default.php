...
?>
<form action="index.php?option=com_eshop&view=options" method="post" name="adminForm" id="adminForm">
	<table width="100%">
		<tr>
			<td align="left">
	...
	<div id="editcell">
		<table class="adminlist table table-striped">
			<thead>
				<tr>
					...
					<th class="text_left" width="40%">
						<?php echo JHtml::_('grid.sort',  JText::_('ESHOP_NAME'), 'b.option_name', $this->lists['order_Dir'], $this->lists['order'] ); ?>				
					</th>
					...
					<th width="10%" class="text_right">
						...
						<?php echo JHtml::_('grid.order',  $this->items , 'filesave.png', 'option.save_order' ); ?>
					</th>
					<th class="text_center" width="5%">
			...
			{
				$row = &$this->items[$i];
				$link 	= JRoute::_( 'index.php?option=com_eshop&task=option.edit&cid[]='. $row->id);
				$checked 	= JHtml::_('grid.id',   $i, $row->id );
				$published 	= JHtml::_('grid.published', $row, $i, 'tick.png', 'publish_x.png', 'option.' );
				?>
				<tr class="<?php echo "row$k"; ?>">
					...
					if (EshopHelper::isJ3())
					{
						...
						<td class="text_center">
							<div class="btn-group">
								<?php
								echo JHtml::_('jgrid.published', $row->published, $i, 'option.');
								echo $this->addDropdownList(JText::_('ESHOP_COPY'), 'copy', $i, 'option.copy');
								echo $this->addDropdownList(JText::_('ESHOP_DELETE'), 'trash', $i, 'option.remove');
								echo $this->renderDropdownList($this->escape($row->option_name));
								?>
							</div>
						</td>
						<?php
					...
					?>
					<td class="text_left">
						<a href="<?php echo $link; ?>"><?php echo $row->option_name; ?></a>
					</td>
					<td class="text_center">
					...
					<td class="order text_right">
						<span><?php echo $this->pagination->orderUpIcon( $i, true, 'option.orderup', 'Move Up', $ordering ); ?></span>
						<span><?php echo $this->pagination->orderDownIcon( $i, $n, true, 'option.orderdown', 'Move Down', $ordering ); ?></span>
