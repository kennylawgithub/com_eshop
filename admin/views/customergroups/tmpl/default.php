...
?>
<form action="index.php?option=com_eshop&view=customergroups" method="post" name="adminForm" id="adminForm">
	<table width="100%">
		<tr>
			<td align="left">
	...
	<div id="editcell">
		<table class="adminlist table table-striped">
			<thead>
				<tr>
					...
					<th class="text_left" width="40%">
						<?php echo JHtml::_('grid.sort',  JText::_('ESHOP_NAME'), 'a.customergroup_name', $this->lists['order_Dir'], $this->lists['order'] ); ?>
					</th>
					<?php
					if (!EshopHelper::isJ3())
					{
			...
			{
				$row = &$this->items[$i];
				$link 	= JRoute::_( 'index.php?option=com_eshop&task=customergroup.edit&cid[]='. $row->id);
				$checked 	= JHtml::_('grid.id',   $i, $row->id );				
				$published 	= JHtml::_('grid.published', $row, $i, 'tick.png', 'publish_x.png', 'customergroup.' );			
				?>
				<tr class="<?php echo "row$k"; ?>">
					...
					if (EshopHelper::isJ3())
					{
						...
						<td class="text_center">
							<div class="btn-group">
								<?php
								echo JHtml::_('jgrid.published', $row->published, $i, 'customergroup.');
								echo $this->addDropdownList(JText::_('ESHOP_COPY'), 'copy', $i, 'customergroup.copy');
								echo $this->addDropdownList(JText::_('ESHOP_DELETE'), 'trash', $i, 'customergroup.remove');
								echo $this->renderDropdownList($this->escape($row->customergroup_name));
								?>
							</div>
						</td>
						<?php
					...
					?>
					<td>																			
						<a href="<?php echo $link; ?>"><?php echo $row->customergroup_name; ?></a>				
					</td>		
					<?php
					if (!EshopHelper::isJ3())
					{
