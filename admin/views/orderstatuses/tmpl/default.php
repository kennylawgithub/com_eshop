...
?>
<form action="index.php?option=com_eshop&view=orderstatuses" method="post" name="adminForm" id="adminForm">
	<table width="100%">
		<tr>
			<td align="left">
	...
	<div id="editcell">
		<table class="adminlist table table-striped">
			<thead>
				<tr>
					...
					<th class="text_left" width="40%">
						<?php echo JHtml::_('grid.sort',  JText::_('ESHOP_NAME'), 'a.orderstatus_name', $this->lists['order_Dir'], $this->lists['order'] ); ?>				
					</th>
					<?php
					if (!EshopHelper::isJ3())
					{
					...
					<th width="10%" class="text_right">
						...
						<?php echo JHtml::_('grid.order',  $this->items , 'filesave.png', 'orderstatus.save_order' ); ?>
					</th>
					<th width="5%" class="text_center">
			...
			{
				$row = &$this->items[$i];
				$link 	= JRoute::_( 'index.php?option=com_eshop&task=orderstatus.edit&cid[]='. $row->id);
				$checked 	= JHtml::_('grid.id',   $i, $row->id );
				$published 	= JHtml::_('grid.published', $row, $i, 'tick.png', 'publish_x.png', 'orderstatus.' );			
				?>
				<tr class="<?php echo "row$k"; ?>">
					...
					if (EshopHelper::isJ3())
					{
						...
						<td class="text_center">
							<div class="btn-group">
								<?php
								echo JHtml::_('jgrid.published', $row->published, $i, 'orderstatus.');
								echo $this->addDropdownList(JText::_('ESHOP_COPY'), 'copy', $i, 'orderstatus.copy');
								echo $this->addDropdownList(JText::_('ESHOP_DELETE'), 'trash', $i, 'orderstatus.remove');
								echo $this->renderDropdownList($this->escape($row->orderstatus_name));
								?>
							</div>
						</td>
						<?php
					...
					?>
					<td class="text_left">
						<a href="<?php echo $link; ?>"><?php echo $row->orderstatus_name; ?></a>
					</td>
					<?php
					if (!EshopHelper::isJ3())
					{
					...
					<td class="order text_right">
						<span><?php echo $this->pagination->orderUpIcon( $i, true, 'orderstatus.orderup', 'Move Up', $ordering ); ?></span>
						<span><?php echo $this->pagination->orderDownIcon( $i, $n, true, 'orderstatus.orderdown', 'Move Down', $ordering ); ?></span>
