...
?>
<form action="index.php?option=com_eshop&view=attributegroups" method="post" name="adminForm" id="adminForm">
	<table width="100%">
		<tr>
			<td align="left">
	...
	<div id="editcell">
		<table class="adminlist table table-striped">
			<thead>
				<tr>
					...
					<th class="text_left" width="50%">
						<?php echo JHtml::_('grid.sort',  JText::_('ESHOP_NAME'), 'b.attributegroup_name', $this->lists['order_Dir'], $this->lists['order'] ); ?>				
					</th>
					<?php
					if (!EshopHelper::isJ3())
					{
					...
					<th width="10%" class="text_right">
						...
						<?php echo JHtml::_('grid.order',  $this->items , 'filesave.png', 'attributegroup.save_order' ); ?>
					</th>
					<th width="5%" class="text_center">
			...
			{
				$row = &$this->items[$i];
				$link 	= JRoute::_( 'index.php?option=com_eshop&task=attributegroup.edit&cid[]='. $row->id);
				$checked 	= JHtml::_('grid.id',   $i, $row->id );
				$published 	= JHtml::_('grid.published', $row, $i, 'tick.png', 'publish_x.png', 'attributegroup.' );			
				?>
				<tr class="<?php echo "row$k"; ?>">
					...
					if (EshopHelper::isJ3())
					{
						...
						<td class="text_center">
							<div class="btn-group">
								<?php
								echo JHtml::_('jgrid.published', $row->published, $i, 'attributegroup.');
								echo $this->addDropdownList(JText::_('ESHOP_COPY'), 'copy', $i, 'attributegroup.copy');
								echo $this->addDropdownList(JText::_('ESHOP_DELETE'), 'trash', $i, 'attributegroup.remove');
								echo $this->renderDropdownList($this->escape($row->attributegroup_name));
								?>
							</div>
						</td>
						<?php
					...
					?>
					<td class="text_left">
						<a href="<?php echo $link; ?>"><?php echo $row->attributegroup_name; ?></a>
					</td>
					<?php
					if (!EshopHelper::isJ3())
					{
					...
					<td class="order text_right">
						<span><?php echo $this->pagination->orderUpIcon( $i, true, 'attributegroup.orderup', 'Move Up', $ordering ); ?></span>
						<span><?php echo $this->pagination->orderDownIcon( $i, $n, true, 'attributegroup.orderdown', 'Move Down', $ordering ); ?></span>
