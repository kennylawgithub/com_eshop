...
<form action="index.php?option=com_eshop&view=labels" method="post" name="adminForm" id="adminForm">
	...
	<div id="editcell">
		<table class="adminlist table table-striped">
			<thead>
				<tr>
					...
					<th class="text_left" width="40%">
						<?php echo JHtml::_('grid.sort',  JText::_('ESHOP_NAME'), 'b.label_name', $this->lists['order_Dir'], $this->lists['order'] ); ?>				
					</th>
					<?php
					if (!EshopHelper::isJ3())
					{
					...
					<th width="10%" class="text_right">
						...
						<?php echo JHtml::_('grid.order',  $this->items , 'filesave.png', 'label.save_order' ); ?>
					</th>
			...
			{
				$row = &$this->items[$i];
				$link 	= JRoute::_( 'index.php?option=com_eshop&task=label.edit&cid[]='. $row->id);
				$checked 	= JHtml::_('grid.id',   $i, $row->id );
				$published 	= JHtml::_('grid.published', $row, $i, 'tick.png', 'publish_x.png', 'label.' );			
				?>
				<tr class="<?php echo "row$k"; ?>">
					...
					if (EshopHelper::isJ3())
					{
						...
						<td class="text_center">
							<div class="btn-group">
								<?php
								echo JHtml::_('jgrid.published', $row->published, $i, 'label.');
								echo $this->addDropdownList(JText::_('ESHOP_COPY'), 'copy', $i, 'label.copy');
								echo $this->addDropdownList(JText::_('ESHOP_DELETE'), 'trash', $i, 'label.remove');
					...
					<td class="order text_right">
						<span><?php echo $this->pagination->orderUpIcon( $i, true, 'label.orderup', 'Move Up', $ordering ); ?></span>
						<span><?php echo $this->pagination->orderDownIcon( $i, $n, true, 'label.orderdown', 'Move Down', $ordering ); ?></span>
