	...
	<div id="editcell">
		<table class="adminlist table table-striped">
			...
			for ($i=0, $n=count( $this->items ); $i < $n; $i++)
			{
				...
				$published 	= JHtml::_('grid.published', $row, $i, 'tick.png', 'publish_x.png', 'discount.' );
				?>
				<tr class="<?php echo "row$k"; ?>">
					...
					if (EshopHelper::isJ3())
					{
						...
						<td class="text_center">
							<div class="btn-group">
								<?php
								echo JHtml::_('jgrid.published', $row->published, $i, 'discount.');
					...
					<td class="text_center">
						<?php 
							if ($row->discount_start_date != $this->nullDate)						
								echo JHtml::_('date', $row->discount_start_date, EshopHelper::getConfigValue('date_format', 'm-d-Y'));
						?>				
					</td>
					<td class="text_center">
						<?php 
							if ($row->discount_end_date != $this->nullDate)
								echo JHtml::_('date', $row->discount_end_date, EshopHelper::getConfigValue('date_format', 'm-d-Y'));
						?>								
					</td>
					<td class="text_center">
						<?php echo $row->id; ?>
