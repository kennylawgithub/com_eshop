 ...
 * @since 1.5
 */
class EShopModelPayment extends EShopModel
{

	/**
	 * Save payment plugin parameter
	 * @see EShopModel::store()
	 */
	function store(&$data)
	{
		$db = $this->getDbo();
		$row = new EShopTable('#__eshop_payments', 'id', $db);
		if ($data['id'])
			$row->load($data['id']);
		if (!$row->bind($data))
		{
	...
	function install()
	{
		...
		$row->store();
		$pluginDir = JPATH_ROOT . '/components/com_eshop/plugins/payment/';
	...
	/**
	 * Remove the selected payment plugin
