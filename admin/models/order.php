		...
		if ($orderStatusChanged)
		{
			...
			if (JRequest::getVar('send_notification_email'))
			{
				...
				if (EshopHelper::getConfigValue('invoice_enable') && EshopHelper::getConfigValue('send_invoice_to_customer') && $row->order_status_id == EshopHelper::getConfigValue('complete_status_id'))
				{
					...
					if (!JFile::exists(JPATH_ROOT . '/media/com_eshop/invoices/' . EShopHelper::formatInvoiceNumber($row->invoice_number) . '.pdf'))
					{
						EshopHelper::generateInvoicePDF(array($row->id));
					}
					$attachment = JPATH_ROOT . '/media/com_eshop/invoices/' . EShopHelper::formatInvoiceNumber($row->invoice_number) . '.pdf';
				}
