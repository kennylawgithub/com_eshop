...
class eshop_auspostpac extends eshop_shipping
{
	...
	function generatePackages($params)
	{
		...
		for ($i = 0; $i < $count; $i++)
		{
			$width = $eshopLength->convert($items[$i]['product_width'], $items[$i]['product_length_id'], EshopHelper::getConfigValue('length_id'));
			$height = $eshopLength->convert($items[$i]['product_height'], $items[$i]['product_length_id'], EshopHelper::getConfigValue('length_id'));
			$length = $eshopLength->convert($items[$i]['product_length'], $items[$i]['product_length_id'], EshopHelper::getConfigValue('length_id'));
			$actual_weight = $eshopWeight->convert($items[$i]['product_weight'], $items[$i]['product_weight_id'], EshopHelper::getConfigValue('weight_id'));
	...
	function addShippingQuote($service, $info, $params)
	{
		...
		$this->quote_data[$code] = array (
				...
				'taxclass_id'	=> $params->get('auspost_pac_tax_class_id'),
	...
	function getTotalWeight()
	{
		...
		{
			$weight += $eshopWeight->convert($product['product_weight'], $product['product_weight_id'], EshopHelper::getConfigValue('weight_id'));
		}
		return $weight;
	}
	...
	function call($call, $args = array(), $query = array(), $format = 'json', $params)
	{
		...
		$url = $protocol.'://auspost.com.au/api/postage/'.$path.$call.'.'.$format.$query_str;
