...
class EShopOmnipayPayment extends OmnipayPayment
{
	...
	protected function beforeRequestSend($request, $row, $data)
	{
		...
		$request->setReturnUrl($siteUrl . 'index.php?option=com_eshop&task=checkout.verifyPayment&payment_method=' . $this->name);
		$request->setNotifyUrl($siteUrl . 'index.php?option=com_eshop&task=checkout.verifyPayment&payment_method=' . $this->name . '&notify=1');
		$request->setAmount(round($data['total'], 2));
	...
	public function getOmnipayCard($data)
	{
		...
		$fieldMappings = array(
				...
				'payment_country_name'		=> 'billingCountry',
				...
				'shipping_country_name'		=> 'shippingCountry',
