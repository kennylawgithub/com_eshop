        	...
        	<div class="row-fluid">
                ...
                <div class="row-fluid">
                	<div class="product-cart clearfix">
						...
						if (($this->item->product_quantity <= 0 && EshopHelper::getConfigValue('allow_notify')) || EshopHelper::getConfigValue('allow_wishlist') || EshopHelper::getConfigValue('allow_compare') || EshopHelper::getConfigValue('allow_ask_question'))
						{
							?>
							<div class="span4">
								<?php
								if ($this->item->product_quantity <= 0 && EshopHelper::getConfigValue('allow_notify'))
								{
									?>
		...
		$('#add-to-cart').bind('click', function() {
			...
			$.ajax({
				...
				success: function(json) {
					...
					if (json['success']) {
						$.ajax({
							url: siteUrl + 'index.php?option=com_eshop&view=cart&layout=popout&format=raw<?php echo EshopHelper::getAttachedLangLink(); ?>',
							dataType: 'html',
		...
		$('#add-to-quote').bind('click', function() {
			...
			$.ajax({
				...
				success: function(json) {
					...
					if (json['success']) {
						$.ajax({
							url: siteUrl + 'index.php?option=com_eshop&view=quote&layout=popout&format=raw<?php echo EshopHelper::getAttachedLangLink(); ?>',
							dataType: 'html',
							success: function(html) {
								$.colorbox({
									overlayClose: true,
